...
class DensityRatioEstimator:
    ...
    def __init__(self, max_iter=5000, num_params=[.1, .2], epsilon=1e-4,
                 cv=3, sigmas=[.01, .1, .25, .5, .75, 1], random_state=None, verbose=0):
        """
    ...
    def fit(self, X_train, X_test, alpha_0=None):
        ...
        if len(self.sigmas) * len(self.num_params) > 1:
                ...
                for sigma in self.sigmas:
                    ...
                    for k in range(1, cv + 1):
                        ...
                        j_scores[(num_param, sigma)][k - 1] = self._fit(X_train=X_train,
                                                                        ...
                                                                        sigma=sigma)
                    j_scores[(num_param, sigma)] = np.mean(
                        j_scores[(num_param, sigma)])

            sorted_scores = sorted([x for x in j_scores.items() if np.isfinite(x[1])], key=lambda x: x[1], reverse=True)
            if len(sorted_scores) == 0:
                warnings.warn(
                    'LCV failed to converge for all values of sigma.')
                return self
            ...
            # best sigma
        self._j = self._fit(X_train=X_train, X_test=X_test_shuffled,
    ...
    def _fit(self, X_train, X_test, num_parameters, sigma, alpha_0=None):
        ...
        dist_train = euclidean_distances(X_train, self._test_vectors, squared=True)
        dist_test = euclidean_distances(X_test, self._test_vectors, squared=True)

        if alpha_0 is None:
            alpha_0 = np.ones(shape=(num_parameters, 1)) / float(num_parameters)

        self._find_alpha(dist_train=dist_train,
                         dist_test=dist_test,
                         num_parameters=num_parameters,
    ...
    def _select_param_vectors(self, X_test, sigma, num_parameters):
        ...
        indices = np.random.choice(
            X_test.shape[0], size=num_parameters, replace=False)
        self._test_vectors = X_test[indices, :].copy()
        self._phi_fitted = True

    def _phi(self, dist, sigma=None):

        if sigma is None:
            sigma = self._sigma

        print('dist shape', dist.shape)
        print('test vectors shape', self._test_vectors.shape)
        if self._phi_fitted:
            return np.exp(-dist / (2 * sigma**2))
        raise Exception('Phi not fitted.')

    def _find_alpha(self, alpha_0, dist_train, dist_test, num_parameters, sigma, epsilon):
        A = np.zeros(shape=(dist_test.shape[0], num_parameters))
        b = np.zeros(shape=(num_parameters, 1))

        A = self._phi(dist_test, sigma)
        b = self._phi(dist_train, sigma).sum(axis=0) / dist_train.shape[0]
        b = b.reshape((num_parameters, 1))

        out = alpha_0.copy()
        for k in range(self.max_iter):
            out += epsilon * np.dot(np.transpose(A), 1. / np.dot(A, out))
            out += b * (((1 - np.dot(np.transpose(b), out)) /
    ...
    def predict(self, X, sigma=None):
        ...
        dist = euclidean_distances(X, self._test_vectors, squared=True)
        if not self._fitted:
            raise Exception('Not fitted!')
        return np.dot(self._phi(dist, sigma=sigma), self._alpha).reshape((X.shape[0],))
